...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const p of plants) {
      ...
      // incomes
      const incomes: any[] = Array.isArray((p as any)?.incomeRecords) ? (p as any).incomeRecords : []
      ...
      // costs
      const costs: any[] = Array.isArray((p as any)?.operationalCosts) ? (p as any).operationalCosts : []
    ...
    entries.sort((a, b) => (a.date === b.date ? (a.kind > b.kind ? -1 : 1) : a.date.localeCompare(b.date)))
    ...
    const byPlantTypeMap = new Map<string, { plantType: string; income: number; expense: number; net: number }>()
    for (const e of entries) {
      const key = e.plantType || "tanpa-jenis"
      if (!byPlantTypeMap.has(key)) byPlantTypeMap.set(key, { plantType: key, income: 0, expense: 0, net: 0 })
    ...
    const byPlantType = Array.from(byPlantTypeMap.values()).sort((a, b) => b.net - a.net)
