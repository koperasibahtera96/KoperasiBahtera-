...
export async function GET(_req: NextRequest) {
  try {
    ...
    // Get all marketing staff with their commission summary
        ...
        const commissionSummary = await CommissionHistory.aggregate([
          { $match: { marketingStaffId: staff._id } },
          {
            $group: {
              ...
              fullInvestments: {
                $sum: { $cond: [{ $eq: ['$paymentType', 'full-investment'] }, 1, 0] }
              },
              cicilanInvestments: {
                $sum: { $cond: [{ $eq: ['$paymentType', 'cicilan-installment'] }, 1, 0] }
              }
            }
          }
        ]);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!validateReferralCode(newReferralCode)) {
      return NextResponse.json({
        error: "Invalid referral code format. Must be 6 alphanumeric characters."
      }, { status: 400 });
    }
