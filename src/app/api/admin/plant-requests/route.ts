...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.user || (session.user.role !== 'admin' && session.user.role !== 'spv_staff')) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { plantId, requestType, deleteReason, historyId, originalDescription, newDescription } = body;
    ...
    if ((requestType === 'update_history' || requestType === 'delete_history') && !historyId) {
      return NextResponse.json(
    ...
    if (requestType === 'update_history' && (!originalDescription || !newDescription)) {
      return NextResponse.json(
        { error: 'Original and new descriptions are required for update requests' },
        { status: 400 }
      );
    }
