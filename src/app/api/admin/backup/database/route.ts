...
export async function POST(_req: NextRequest) {
  try {
    ...
    console.log(
      `Found ${collections.length} collections via listCollections:`,
    ...
    if (collections.length === 0) {
      console.log(
        "No collections found via listCollections, trying mongoose.connection.models..."
      );
      ...
      console.log(
        `Found ${collections.length} collections from models:`,
        collections.map((c: any) => c.name)
      );
    }
    ...
    for (const collection of collections) {
      ...
      try {
        ...
        console.log(
          `Collection ${collectionName} has ${data.length} documents`
        );
