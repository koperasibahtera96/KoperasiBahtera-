...
export async function GET() {
  try {
    ...
    const userIds = [...new Set(cicilanPayments.map(p => p.userId.toString()))];
    ...
    const contractIds = [...new Set(cicilanPayments.map(p => p.cicilanOrderId))];

    // Get contract information to get the actual total amounts
    const contracts = await Contract.find({ contractId: { $in: contractIds } });
    const contractsMap = new Map(contracts.map(contract => [contract.contractId, contract]));
    ...
    Array.from(contractGroups.entries()).forEach(([cicilanOrderId, payments]) => {
      const sortedPayments = payments.sort((a: any, b: any) => a.installmentNumber - b.installmentNumber);
      ...
      sortedPayments.forEach((payment: any) => {
        ...
        // Format due date
        const tanggalJatuhTempo = new Date(payment.dueDate).toLocaleDateString("id-ID");

        // Format payment date (if paid)
        let tanggalBayar = "-";
        if (payment.transactionStatus === "settlement" && payment.settlementTime) {
          tanggalBayar = new Date(payment.settlementTime).toLocaleDateString("id-ID");
        }
        ...
        let keterlambatan = 0;
        if (payment.transactionStatus !== "settlement" && new Date(payment.dueDate) < new Date()) {
          const diffTime = new Date().getTime() - new Date(payment.dueDate).getTime();
        ...
        const actualTotalAmount = contract?.totalAmount || (payment.installmentAmount * payment.totalInstallments);
        const totalKontrak = `Rp. ${actualTotalAmount.toLocaleString("id-ID")}`;
        const nominalCicilan = `Rp. ${payment.installmentAmount.toLocaleString("id-ID")}`;
    ...
    const headerInfo = [
      ...
      [""],
      ["DAFTAR CICILAN"],
      ["No.", "No. Kontrak", "Nama User", "Jenis Paket", "Total Kontrak", "Jenis Cicilan", "Cicilan ke-", "Nominal Cicilan (Rp)", "Tanggal Jatuh Tempo", "Tanggal Bayar", "Status Cicilan", "Keterlambatan (Hari)"]
    ];
    ...
    for (let row = range.s.r; row <= range.e.r; row++) {
      for (let col = range.s.c; col <= range.e.c; col++) {
        ...
        // Section headers
        if (worksheetData[row][0] === "DAFTAR CICILAN" ||
        ...
        if (row > 8 && (col === 0 || col === 6 || col === 11)) { // No., Cicilan ke-, Keterlambatan
          worksheet[cellRef].s.alignment = { horizontal: 'center' };
        }
      }
    }
    ...
    try {
      ...
      // Merge "DAFTAR CICILAN" across all 12 columns
        ...
        for (let c = 0; c <= 11; c++) {
          ...
          if (r === 3 || r === 4 || r === 7) {
            worksheet[cellRef].s.border = {
              ...
              right: c === 11 ? { style: 'thin', color: { rgb: '000000' } } : undefined,
            };
          }
        }
      }
    ...
    return new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="laporan-cicilan-${new Date().toISOString().split('T')[0]}.xlsx"`
      }
    });
