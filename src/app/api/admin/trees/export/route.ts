...
export async function GET() {
  try {
    ...
    // Get all unique user IDs from investors to fetch user data
    const userIds = [
      ...new Set(investors.map((inv) => inv.userId).filter((id) => id)),
    ];
    ...
    plantInstances.forEach((instance) => {
      ...
      for (const investor of investors) {
        const hasMatchingInvestment = investor.investments?.some(
          (investment: any) =>
        ...
        if (hasMatchingInvestment) {
          ...
          // Get userCode from User model if available
          ...
          const matchingInvestment = investor.investments?.find(
            (investment: any) =>
              investment.plantInstanceId &&
              investment.plantInstanceId.toString() === instance._id.toString()
          );
          if (matchingInvestment?.investmentDate) {
            purchaseDate = new Date(
              matchingInvestment.investmentDate
      ...
      const kondisi =
        instance.status?.toLowerCase() === "sakit"
          ? "Sakit"
          : instance.status?.toLowerCase() === "mati"
          ? "Mati"
          : "Sehat";

      // Plant type label
      const jenisPacket =
        plantTypeLabels[instance.plantType] || instance.plantType;
    ...
    for (let row = range.s.r; row <= range.e.r; row++) {
      for (let col = range.s.c; col <= range.e.c; col++) {
        ...
        const shouldHaveBorders = !(
          ...
          (worksheetData[row][0] &&
            worksheetData[row][0].toString().startsWith("Dibuat pada:")) ||
          worksheetData[row][0] === "" ||
        ...
        if (
          row === 0 ||
          worksheetData[row][0] === "KOPERASI BINTANG MERAH SEJAHTERA"
        ) {
    ...
    try {
      ...
      // Merge company header rows (0..4) across all 11 columns for the detailed table
      for (let r = 0; r <= 4; r++) {
        worksheet["!merges"].push({ s: { r, c: 0 }, e: { r, c: 10 } });
      }
      ...
      // Merge "INSTANSI SELURUH TANAMAN" across all 11 columns
      if (instansiRow >= 0) {
        worksheet["!merges"].push({
          s: { r: instansiRow, c: 0 },
          e: { r: instansiRow, c: 10 },
        });
      }
      ...
      for (let r = 0; r <= maxRow; r++) {
        for (let c = 0; c <= 10; c++) {
          ...
          const isRincianSection =
            r >= 8 && r < (instansiRow >= 0 ? instansiRow - 2 : maxRow);
      ...
      for (let c = 0; c <= 1; c++) {
        ...
        worksheet[cellRef].s.border = {
          ...
          left:
            c === 0 ? { style: "thin", color: { rgb: "000000" } } : undefined,
          right:
            c === 1 ? { style: "thin", color: { rgb: "000000" } } : undefined,
        };
      }
    ...
    return new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="laporan-data-pohon-${
