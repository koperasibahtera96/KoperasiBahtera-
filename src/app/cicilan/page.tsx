...
export default function CicilanPage() {
  ...
  const handleUploadProof = async (paymentId: string, file: File, description: string) => {
    ...
    try {
      ...
      showError('Kesalahan', 'Terjadi kesalahan saat mengunggah bukti pembayaran');
    } finally {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'pending': return 'bg-gradient-to-r from-[#324D3E]/10 to-[#4C3D19]/10 text-[#324D3E] border border-[#324D3E]/20';
      case 'submitted': return 'bg-gradient-to-r from-yellow-100 to-amber-100 text-amber-800 border border-amber-200';
      case 'approved': return 'bg-gradient-to-r from-emerald-100 to-green-100 text-emerald-800 border border-emerald-200';
      case 'rejected': return 'bg-gradient-to-r from-red-100 to-rose-100 text-red-800 border border-red-200';
      case 'overdue': return 'bg-gradient-to-r from-red-100 to-rose-100 text-red-800 border border-red-200';
      case 'not_created': return 'bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-600 border border-blue-200';
      default: return 'bg-gradient-to-r from-[#324D3E]/10 to-[#4C3D19]/10 text-[#324D3E] border border-[#324D3E]/20';
    }
  };
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'not_created': return 'Belum Tersedia';
  ...
  const canSubmitProof = (installment: Installment) => {
    ...
    return (installment.status === 'pending' || installment.status === 'rejected') && (installment as any).exists !== false;
  };
  ...
  const getPortfolioStats = () => {
    ...
    const totalAmount = groupedInstallments.reduce((sum, group) => sum + group.totalAmount, 0);
    const totalPaid = groupedInstallments.reduce((sum, group) => {
      return sum + group.installments
        .filter(inst => inst.status === 'approved')
        .reduce((installSum, inst) => installSum + inst.amount, 0);
    }, 0);

    const allInstallments = groupedInstallments.flatMap(group => group.installments);
    const overdueCount = allInstallments.filter(inst =>
      isOverdue(inst.dueDate) && inst.status === 'pending'
    ).length;
    const upcomingCount = allInstallments.filter(inst =>
      !isOverdue(inst.dueDate) && inst.status === 'pending'
    ).length;

    return { totalInvestments, totalAmount, totalPaid, overdueCount, upcomingCount };
  };
  ...
  const getFilteredInstallments = () => {
    let filtered = groupedInstallments.filter(group => {
      if (searchTerm) {
        return group.productName.toLowerCase().includes(searchTerm.toLowerCase());
      }
      return true;
    });
    ...
    if (filter !== 'all') {
      filtered = filtered.filter(group => {
        ...
        const hasOverdue = group.installments.some(inst =>
          isOverdue(inst.dueDate) && inst.status === 'pending'
        );
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 font-poppins">Memuat data cicilan...</p>
          </div>
        </div>
      </>
    );
        ...
        <div className="max-w-7xl mx-auto px-4 mt-12">
          ...
          {groupedInstallments.length === 0 ? (
            <div className="text-center py-12 bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl">
              ...
              <button
                onClick={() => router.push('/landing#investasi')}
            ...
            <div className="space-y-12">
              {getFilteredInstallments()
                .sort((a, b) => new Date(b.installments[0]?.createdAt || 0).getTime() - new Date(a.installments[0]?.createdAt || 0).getTime())
                .map((group) => (
                  <div key={group.cicilanOrderId} className="bg-gradient-to-br from-[#FFFCE3]/95 to-white/95 backdrop-blur-sm rounded-3xl shadow-xl border-2 border-[#324D3E]/20 p-8 hover:shadow-2xl transition-all duration-300">
                    {/* Cicilan Header */}
                    <div className="mb-8">
                      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-4 gap-4">
                        <div className="flex-1">
                          <h2 className="text-2xl font-bold text-[#324D3E] font-poppins">{group.productName}</h2>
                          <p className="text-sm text-gray-600 font-poppins">Order ID: {group.cicilanOrderId}</p>
                        ...
                        <div className="lg:text-right">
                          <div className="text-sm text-gray-600 font-poppins">Progress Pembayaran</div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {group.installments
                          ...
                          .filter((installment) => {
                            ...
                            if (installment.status === 'approved' || installment.status === 'submitted' || installment.status === 'rejected') {
                              return true;
                            }

                            // Show current pending installment (first unpaid)
                            if (installment.status === 'pending' && installment.exists !== false) {
                              const pendingInstallments = group.installments.filter(inst =>
                                inst.status === 'pending' && inst.exists !== false
                              );
                              return installment.installmentNumber === Math.min(...pendingInstallments.map(inst => inst.installmentNumber));
                            }

                            // Show next placeholder installment (only one)
                            if (installment.status === 'not_created') {
                              const notCreatedInstallments = group.installments.filter(inst => inst.status === 'not_created');
                              return installment.installmentNumber === Math.min(...notCreatedInstallments.map(inst => inst.installmentNumber));
                            }
                          ...
                          .map((installment) => {
                            ...
                            else if (overdue && installment.status === 'pending' && !installment.proofImageUrl) {
                            ...
                            return (
                              <div
                                ...
                                className={`border-2 rounded-2xl p-6 transition-all duration-300 hover:shadow-xl backdrop-blur-sm hover:scale-105 ${effectiveStatus === 'approved' ? 'bg-gradient-to-br from-emerald-50/90 to-green-50/90 border-emerald-200' :
                                  effectiveStatus === 'submitted' ? 'bg-gradient-to-br from-yellow-50/90 to-amber-50/90 border-amber-200' :
                                    effectiveStatus === 'overdue' ? 'bg-gradient-to-br from-red-50/90 to-rose-50/90 border-red-200' :
                                      effectiveStatus === 'rejected' ? 'bg-gradient-to-br from-red-50/90 to-rose-50/90 border-red-200' :
                              ...
                              >
                                <div className="flex justify-between items-start mb-3">
                                  <div>
                                    ...
                                    <div className="text-sm text-gray-600 font-poppins">
                                      Jatuh tempo: {installment.dueDate ? formatDate(installment.dueDate) : 'Akan ditentukan'}
                                    </div>
                                  </div>
                                  <span className={`px-3 py-2 text-xs font-bold rounded-full ${getStatusColor(effectiveStatus)}`}>
                                ...
                                {canSubmitProof(installment) && (
                                  <button
                                    onClick={() => setUploadModal({ isOpen: true, installment })}
...
interface InstallmentProofUploadModalProps {
  ...
  installment: Installment | null;
  onClose: () => void;
  onUpload: (paymentId: string, file: File, description: string) => Promise<void>;
  isUploading: boolean;
}

function InstallmentProofUploadModal({ isOpen, installment, onClose, onUpload, isUploading }: InstallmentProofUploadModalProps) {
      ...
      <div className="bg-[#FFFCE3] rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto border-2 border-[#324D3E]/20">
        <div className="p-6">
          ...
          <div className="bg-gradient-to-r from-[#324D3E]/10 to-[#4C3D19]/10 p-3 rounded-2xl mb-4">
            <div className="text-sm text-gray-600 font-poppins">Angsuran #{installment.installmentNumber}</div>
            <div className="text-lg font-semibold text-[#324D3E] font-poppins">Rp {installment.amount.toLocaleString('id-ID')}</div>
            <div className="text-sm text-gray-600 font-poppins">Jatuh tempo: {installment.dueDate ? formatDate(installment.dueDate) : 'Akan ditentukan'}</div>
          </div>
...
function PortfolioOverview({ stats }: PortfolioOverviewProps) {
  const completionPercentage = stats.totalAmount > 0 ? (stats.totalPaid / stats.totalAmount) * 100 : 0;

  return (
    <div className="bg-white/95 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 p-6">
      <h2 className="text-2xl font-bold text-[#324D3E] font-poppins mb-6">Portfolio Overview</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-4">
          ...
          <div className="text-sm text-gray-600 font-poppins">Nilai Investasi</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-4">
          ...
          <div className="text-sm text-gray-600 font-poppins">Sudah Dibayar</div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-4">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="text-xs text-gray-600 font-poppins">Terlambat</div>
            </div>
            <div>
              <div className="text-lg font-bold text-orange-600 font-poppins">{stats.upcomingCount}</div>
              <div className="text-xs text-gray-600 font-poppins">Mendatang</div>
            </div>
          </div>
        </div>
      </div>
...
function SearchAndFilter({ searchTerm, setSearchTerm, filter, setFilter, stats }: SearchAndFilterProps) {
  const filters = [
    { key: 'all', label: 'Semua', count: stats.totalInvestments },
    { key: 'active', label: 'Aktif', count: stats.totalInvestments - Math.floor(stats.totalPaid / stats.totalAmount * stats.totalInvestments) },
    { key: 'overdue', label: 'Terlambat', count: stats.overdueCount > 0 ? 1 : 0 },
    { key: 'completed', label: 'Selesai', count: Math.floor(stats.totalPaid / stats.totalAmount * stats.totalInvestments) },
  ];
        ...
        <div className="flex flex-wrap gap-2">
          {filters.map((filterOption) => (
            <button
              ...
              className={`px-4 py-2 rounded-full font-medium transition-all duration-300 font-poppins text-sm ${filter === filterOption.key
