...
export default function LaporanPage() {
  ...
  const downloadAllInvestorsXLSX = () => {
    ...
    const summarySheetData = [
      ...
      ...reportData.reports.map((report, reportIdx) => {
  ...
  const [detailedPlantData, setDetailedPlantData] = useState<
    Record<string, any[]>
  >({});
  const [_loadingPlantAges, setLoadingPlantAges] = useState<
    Record<string, boolean>
  >({});
  const [itemsPerPage] = useState(5);
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState("");

  // Format number helper (used in PDF/XLSX exports)
  const _formatNumber = (num: number) => {
    return num.toLocaleString("id-ID");
  };

  // Format detailed age helper (used in PDF/XLSX exports)
  ...
  // Individual investor report download functions (kept for future use/direct exports)
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  // Fetch detailed plant ages for specific investor (used by PDF/XLSX exports)
  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/admin/laporan?page=1&limit=1`);
        if (response.ok) {
          const result = await response.json();
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Data Section - Only reloads when search changes */}
        <LaporanDataSection
