  ...
  const handleExportCSV = async () => {
    try {
      ...
      const allRows = relevantPlants
        .flatMap((plant) =>
          (isExpense ? plant.operationalCosts || [] : plant.incomeRecords || [])
            .filter((it) => {
              const d = new Date(it.date);
              const yearMatch = d.getFullYear() === selectedYear;
              const monthMatch =
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <ChevronDown
                    className={getThemeClasses(
                      "absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-slate-400 dark:text-gray-300 pointer-events-none",
                      "!text-[#4c1d1d]"
                    )}
                  />
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        strokeWidth={3}
                      >
                        {(isExpense ? piePrimary : piePrimary).map(
                          (entry, index) => (
