...
export default function InvestorDetailPage({ params }: { params: Promise<{ userId: string }> }) {
    ...
    return (
      <AdminLayout>
        <div className="p-8 text-center">
          ...
          <p className="text-[#889063] dark:text-gray-300 font-[family-name:var(--font-poppins)]">Memuat data investor...</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  if (!investorDetail) {
    return (
      <AdminLayout>
        <div className="p-8 text-center">
          <div className="text-[#889063] dark:text-gray-300 text-4xl mb-4">❌</div>
          <h3 className="text-lg font-semibold text-[#324D3E] dark:text-white mb-2 font-[family-name:var(--font-poppins)]">Data investor tidak ditemukan</h3>
          <button
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-[#324D3E] dark:text-white font-[family-name:var(--font-poppins)]">Detail Investor</h1>
            <p className="text-[#889063] dark:text-gray-300 mt-2 text-sm sm:text-base">Kelola cicilan investasi dari {investorDetail.userInfo.fullName}</p>
          </div>
        </div>
        ...
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-lg border border-[#324D3E]/10 dark:border-gray-700 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl sm:text-2xl font-bold text-[#324D3E] dark:text-white font-[family-name:var(--font-poppins)] truncate">{investorDetail.userInfo.fullName}</h2>
              <p className="text-[#889063] dark:text-gray-300 text-sm sm:text-base truncate">{investorDetail.userInfo.email}</p>
              <p className="text-sm text-[#889063]/70 dark:text-gray-400">{investorDetail.userInfo.phoneNumber}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="bg-gradient-to-r from-[#324D3E]/5 to-[#324D3E]/10 dark:from-gray-700/50 dark:to-gray-700 p-3 sm:p-4 rounded-xl border border-[#324D3E]/10 dark:border-gray-600">
              <div className="text-xs sm:text-sm text-[#889063] dark:text-gray-300 font-[family-name:var(--font-poppins)]">Total Investasi</div>
              <div className="text-xl sm:text-2xl font-bold text-[#324D3E] dark:text-white">{investorDetail.totalInvestments}</div>
            </div>
            <div className="bg-gradient-to-r from-[#4C3D19]/5 to-[#4C3D19]/10 dark:from-emerald-800/30 dark:to-emerald-900/30 p-3 sm:p-4 rounded-xl border border-[#4C3D19]/10 dark:border-emerald-700/50">
              <div className="text-xs sm:text-sm text-[#889063] dark:text-gray-300 font-[family-name:var(--font-poppins)]">Nilai Investasi</div>
            ...
            <div className="bg-gradient-to-r from-[#889063]/5 to-[#889063]/10 dark:from-blue-800/30 dark:to-blue-900/30 p-3 sm:p-4 rounded-xl border border-[#889063]/10 dark:border-blue-700/50">
              <div className="text-xs sm:text-sm text-[#889063] dark:text-gray-300 font-[family-name:var(--font-poppins)]">Sudah Dibayar</div>
            ...
            <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/30 dark:to-yellow-800/30 p-3 sm:p-4 rounded-xl border border-yellow-200 dark:border-yellow-700/50">
              <div className="text-xs sm:text-sm text-yellow-700 dark:text-yellow-300 font-[family-name:var(--font-poppins)]">Perlu Review</div>
              <div className="text-xl sm:text-2xl font-bold text-yellow-600 dark:text-yellow-200">{investorDetail.pendingReviews}</div>
              {investorDetail.overdueCount > 0 && (
                <div className="text-xs sm:text-sm text-red-600 dark:text-red-400 mt-1 font-medium">{investorDetail.overdueCount} Terlambat</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {investorDetail.cicilanGroups
            ...
            .map((group) => (
              <div key={group.cicilanOrderId} className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-lg border border-[#324D3E]/10 dark:border-gray-700 p-4 sm:p-6">
                {/* Group Header */}
                <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-6">
                  <div className="flex-1">
                    <h3 className="text-lg sm:text-xl font-bold text-[#324D3E] dark:text-white font-[family-name:var(--font-poppins)]">{group.productName}</h3>
                    <p className="text-xs sm:text-sm text-[#889063] dark:text-gray-300 break-all">Order ID: {group.cicilanOrderId}</p>
                  ...
                  {group.installments
                    ...
                    .map((installment) => {
                      ...
                      return (
                        <div
                          key={installment._id}
                          className={`border-2 rounded-xl p-3 sm:p-4 transition-all duration-300 hover:shadow-md ${effectiveStatus === 'approved' ? 'bg-gradient-to-r from-[#4C3D19]/5 to-[#4C3D19]/10 dark:from-green-800/30 dark:to-green-900/30 border-[#4C3D19]/20 dark:border-green-700/50' :
                            effectiveStatus === 'submitted' ? 'bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/30 dark:to-yellow-800/30 border-yellow-200 dark:border-yellow-700/50' :
                              effectiveStatus === 'overdue' ? 'bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-800/30 border-red-200 dark:border-red-700/50' :
                                effectiveStatus === 'rejected' ? 'bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-800/30 border-red-200 dark:border-red-700/50' :
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="bg-gradient-to-r from-[#324D3E]/10 to-[#4C3D19]/10 dark:from-gray-700/50 dark:to-gray-700 p-3 sm:p-4 rounded-2xl mb-4 sm:mb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm">
              <div>
                <span className="text-gray-600 dark:text-gray-300 font-poppins">Angsuran ke:</span>
                <div className="font-semibold text-[#324D3E] dark:text-white font-poppins">#{installment.installmentNumber}</div>
              </div>
              <div>
                <span className="text-gray-600 dark:text-gray-300 font-poppins">Jumlah:</span>
                <div className="font-semibold text-[#324D3E] dark:text-white font-poppins">Rp {installment.amount.toLocaleString('id-ID')}</div>
              </div>
              <div>
                <span className="text-gray-600 dark:text-gray-300 font-poppins">Jatuh Tempo:</span>
                <div className="font-semibold text-[#324D3E] dark:text-white font-poppins text-sm">{formatDate(installment.dueDate)}</div>
              </div>
              <div>
                <span className="text-gray-600 dark:text-gray-300 font-poppins">Order ID:</span>
                <div className="font-medium text-xs text-[#4C3D19] dark:text-emerald-300 font-poppins break-all">{installment.orderId || installment._id}</div>
              </div>
            </div>
          </div>
          ...
          {installment.proofImageUrl && (
            <div className="mb-4 sm:mb-6">
              <h4 className="font-semibold text-[#324D3E] dark:text-white mb-2 sm:mb-3 font-poppins">Bukti Pembayaran</h4>
          ...
          {installment.proofDescription && (
            <div className="mb-4 sm:mb-6">
              <h4 className="font-semibold text-[#324D3E] dark:text-white mb-2 font-poppins">Keterangan dari User</h4>
              <div className="bg-gradient-to-r from-blue-50/80 to-indigo-50/80 dark:from-blue-900/30 dark:to-indigo-800/30 p-3 sm:p-4 rounded-2xl border border-blue-200/50 dark:border-blue-700/50">
                <p className="text-gray-700 dark:text-gray-300 font-poppins text-sm sm:text-base">{installment.proofDescription}</p>
              </div>
            </div>
          )}
