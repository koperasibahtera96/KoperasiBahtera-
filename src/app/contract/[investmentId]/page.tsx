...
export default function ContractPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text(`Email: ${contractData.investor.email}`, leftMargin + 5, yPosition + 26);
  ...
  if (!contractData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#324D3E]/10 via-white to-[#4C3D19]/10 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-4">Data Kontrak Tidak Ditemukan</h1>
          <p className="text-gray-600">Silakan hubungi customer service kami.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#324D3E]/10 via-white to-[#4C3D19]/10 py-12">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#324D3E] mb-2">Surat Perjanjian Hak Kepemilikan Pohon</h1>
          ...
          <div className="p-8">
            ...
            <div className="border-b border-gray-200 pb-6 mb-6">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold text-[#324D3E]">Nomor Kontrak: {contractData.contractNumber}</h2>
                  <p className="text-[#889063]">Tanggal: {new Date(contractData.contractDate).toLocaleDateString('id-ID')}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-[#889063]">ID Investasi</p>
                  <p className="font-semibold text-[#324D3E]">{contractData.investment.investmentId}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <h3 className="text-lg font-semibold text-[#324D3E] mb-4">Data Investor</h3>
                <div className="space-y-2">
                  <p><span className="text-[#889063]">Nama:</span> <span className="font-medium">{contractData.investor.name}</span></p>
                  <p><span className="text-[#889063]">Email:</span> <span className="font-medium">{contractData.investor.email}</span></p>
                  {contractData.investor.phoneNumber && (
                    <p><span className="text-[#889063]">Telepon:</span> <span className="font-medium">{contractData.investor.phoneNumber}</span></p>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <p><span className="text-[#889063]">Produk:</span> <span className="font-medium">{contractData.investment.productName}</span></p>
                  <p><span className="text-[#889063]">Jenis Pohon:</span> <span className="font-medium">{contractData.plantInstance.plantType.toUpperCase()}</span></p>
                  <p><span className="text-[#889063]">Instansi:</span> <span className="font-medium">{contractData.plantInstance.instanceName}</span></p>
                  <p><span className="text-[#889063]">Total:</span> <span className="font-medium">Rp {contractData.investment.totalAmount.toLocaleString('id-ID')}</span></p>
                  <p><span className="text-[#889063]">ROI:</span> <span className="font-medium">{contractData.plantInstance.baseAnnualROI}%/tahun</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#324D3E]/5 rounded-xl p-6 mb-8">
              <h3 className="text-lg font-semibold text-[#324D3E] mb-4">Ringkasan Ketentuan</h3>
              <ul className="space-y-2 text-sm text-[#324D3E]">
                <li>• Investor berhak atas hasil panen sesuai persentase investasi</li>
                <li>• Hak kepemilikan berlaku setelah pembayaran lunas dan penandatanganan kontrak</li>
                <li>• Investor berhak mendapat laporan perkembangan pohon secara berkala</li>
              ...
              <div className="border-2 border-[#324D3E]/20 rounded-xl p-4 mb-4">
                <SignatureCanvas
                  ...
                  canvasProps={{
                    width: 400,
                    height: 200,
                    className: 'signature-canvas w-full h-48 border border-gray-300 rounded-lg',
