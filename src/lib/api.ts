...
export const plantInstancesAPI = {
  ...
  create: async (
    plantInstance: Omit<PlantInstance, "id">
  ...
  update: async (
    id: string,
    plantInstance: Partial<PlantInstance>
  ): Promise<PlantInstance> => {
    const response = await fetch(`${API_BASE_URL}/plantInstances/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  addOperationalCost: async (
    ...
    return plantInstancesAPI.update(plantId, {
      operationalCosts: plant.operationalCosts,
    });
  },
  ...
  addIncomeRecord: async (
    ...
    return plantInstancesAPI.update(plantId, {
      incomeRecords: plant.incomeRecords,
    });
  },
};
