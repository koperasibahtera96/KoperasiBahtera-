...
if (!WHATSAPP_SERVICE_SECRET) {
  console.warn('WHATSAPP_SERVICE_SECRET not set, WhatsApp functionality will not work');
}
...
export const sendWhatsAppMessage = async (to: string, message: string): Promise<WhatsAppMessageResult> => {
  try {
    ...
    console.error('Error calling WhatsApp service for send message:', error.message);
    return {
      success: false,
      error: error.message,
    };
...
export const generateWhatsAppQR = async (whatsappNumber: string): Promise<{ success: boolean; error?: string }> => {
  try {
    ...
    console.error('Error calling WhatsApp service for QR generation:', error.message);
    return {
      success: false,
      error: error.message,
    };
...
export const getWhatsAppQR = async (whatsappNumber: string): Promise<QRCodeResponse> => {
  try {
    ...
    console.error('Error calling WhatsApp service for QR retrieval:', error.message);
    return {
      success: false,
      error: error.message,
    };
...
export const getWhatsAppConnectionStatus = async (whatsappNumber: string): Promise<WhatsAppStatus> => {
  try {
    const params = new URLSearchParams({ whatsappNumber });
...
export const removeWhatsAppAuth = async (whatsappNumber: string): Promise<{ success: boolean; error?: string }> => {
  try {
    ...
    console.error('Error calling WhatsApp service for auth removal:', error.message);
    return {
      success: false,
      error: error.message,
    };
